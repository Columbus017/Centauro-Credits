...
- **List screens page, and the filters are the URL.** `lib/pagination.ts` holds the arithmetic and `components/{pagination,list-filters}.tsx` the controls; the server renders from `?q=`, `?page=` and the filter params, so a filtered list can be linked and reloaded. Never render a whole table again: the real book is 57,131 payments, which unpaged was 297 MB of HTML. A summary tile above a paged table is either a SQL aggregate over the filtered set (payments) or a figure over the live portfolio, which is bounded (credits, clients) — never a `reduce()` over every row.
- **Dropdowns take `*Options()` queries, not list queries.** `listCollectors()` walks every credit and ledger entry to build each collector's book; `collectorOptions()` selects id and name. Filters over history pass `includeInactive: true`, or a retired collector's years of records become unreachable.
